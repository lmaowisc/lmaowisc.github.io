...
- `research.qmd` and `research.css`: Research page content and styling. Use a full-width layout without a contents sidebar, aligned grant rows under Active Funding and Past Funding, and two columns for Collaborative Research. Match homepage typography and muted red metadata separators. CASCADE belongs under active funding with Lu Mao identified as UW sub-award PI; all PI labels use regular weight. Agency images use `funding-agency-logo` to blend their white backgrounds into the page. The official NCI SVG and its source note are in `images/`.
