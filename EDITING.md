...
- `packages/index.qmd` and `packages/packages.css`: compact badge-and-text entries for WR and rmt, with Website / CRAN / GitHub links. The editable SVG badges in `packages/images/` have comparable typography, illustration style, and visual weight to the course badges; their colors and details can differ. Keep the two package badges equal in size and preserve lowercase `rmt`. Increment the stylesheet version after CSS changes. The former `packages/packages.yml` table is no longer used by this page.
