...
- `research.qmd` and `research.css`: Research page content and styling. Use a full-width layout without a contents sidebar, aligned grant rows under Active Funding and Past Funding, and two columns for Collaborative Research. Match homepage typography and muted red metadata separators; keep modest extra space between sections and indent the collaboration topic lists. CASCADE belongs under active funding with Lu Mao identified as UW sub-award PI; all PI labels use regular weight. Its description uses middle-dot separators without a forced line break. Agency images use `funding-agency-logo` to blend their white backgrounds into the page. Keep the vertical NHLBI and NCI variants at equal widths and vertically centered beside the grant descriptions. The user-supplied NCI square logo and source note are in `images/`.
